...
namespace CardAPILib.InterfaceCL
{
    ...
    /// 
    /// </summary>
    public class ExternalAuthentificate
    {
        ...
        /// <returns></returns>
        public int ExternalAuth(string keyValue)
        {
            ...
            {
                var keyByte = Enumerable.Range(0, keyValue.Length)
                .Where(x => x % 2 == 0)
                .Select(x => Convert.ToByte(keyValue.Substring(x, 2), 16))
                .ToArray();

                key = keyByte;

                if (!keyValue.Equals("404142434445464748494A4B4C4D4E4F"))
                {
                    //Default Key

                        if (CallApduCommandLe(0xC0, 0xB0, 0x05, 0xB6, 0x10, 16, ReaderInfo, iCard) != 0)
                        {
                            //_logService.Info("-3-");
                            //return -3;
                        }

                        //4 new byte[16] { 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F }
                        if (CallApduCommand(0xC0, 0xD6, 0x05, 0xB6, keyByte, ReaderInfo, iCard) != 0)
                        {
                            //_logService.Info("-4-");
                            //return -4;
                        }
                        ...
                        //5
                        if (CallApduCommandLe(0xC0, 0xB0, 0x05, 0xE6, 0x10, 16, ReaderInfo, iCard) != 0)
                        {
                            //_logService.Info("-5-");
                            //return -5;
                        }

                        //6  new byte[16] { 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F }
                        if (CallApduCommand(0xC0, 0xD6, 0x05, 0xE6, keyByte, ReaderInfo, iCard) != 0)
                        {
                            //_logService.Info("-6-");
                            //return -6;
                        }
                        ...
                        //7
                        if (CallApduCommandLe(0xC0, 0xB0, 0x06, 0x16, 0x10, 16, ReaderInfo, iCard) != 0)
                        {
                            //_logService.Info("-7-");
                            //return -7;
                        }

                        //8 new byte[16] { 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F }
                        if (CallApduCommand(0xC0, 0xD6, 0x06, 0x16, keyByte , ReaderInfo, iCard) != 0)
                        {
                            //_logService.Info("-8-");
                            //return -8;
                        }
        ...
        private int CallApduCommandLe(byte Cla, byte Ins, byte P1, byte P2, byte Le, uint _recieveLengt, string readerInfo, CardNative _cardx)
        {
            ...
            if (apduResp == null)
            {
                ...
                //_logService.Info("NULL !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" + Thread.CurrentThread.Name);

                return -1;
            }

            //_logService.Info("Status: " + apduResp.Status.ToString() + Thread.CurrentThread.Name);
        ...
        private string ReaderInfo { get; set; }
